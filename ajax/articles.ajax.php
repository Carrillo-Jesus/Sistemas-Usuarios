...
if (isset($_POST['form_post']) && $_POST['form_post'] == "post") {
	...
	if (!empty($_POST['title']) && !empty($_POST['reg_description'])) {
		...
		$query = "INSERT INTO ud_articles(title, url, description_a, images, ahutor, visitors, comments, like_points) VALUES (?,?,?,?,?,?,?,?)";

		$stmt = $conn->prepare($query);
		if ($stmt === false) {
			echo 'Error en la preparación de la consulta: ' . $conn->error;
		}
		$stmt->bind_param('ssssiiii', $title, $url, $description, $image, $id_user, $visitors, $comments, $like_points);
